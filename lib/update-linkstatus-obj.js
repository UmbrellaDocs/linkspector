...
function updateLinkStatusObj(astNodes, linkStatus) {
  ...
  updatedLinkStatus.sort((a, b) => {
    // Provide fallback values for sorting if position or start is undefined
    const aLine = a.position && a.position.start ? a.position.start.line : 0;
    const bLine = b.position && b.position.start ? b.position.start.line : 0;
    const aCol = a.position && a.position.start ? a.position.start.column : 0;
    const bCol = b.position && b.position.start ? b.position.start.column : 0;
