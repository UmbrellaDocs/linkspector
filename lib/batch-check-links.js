...
async function processLink(link, page, aliveStatusCodes, httpHeaders) {
  ...
  try {
    if (isUrl(link.url)) {
      const headers = httpHeaders.find((header) =>
        header.url.includes(link.url)
      )?.headers || {}
...
async function checkHyperlinks(nodes, options = {}, filePath) {
  const { batchSize = 100, retryCount = 3, aliveStatusCodes, httpHeaders = [] } = options
    ...
    for (let i = 0; i < tempArray.length; i += batchSize) {
      ...
      const promises = batch.map(async (link) => {
        ...
        while (retryCountLocal < retryCount) {
          try {
            linkStatus = await processLink(link, page, aliveStatusCodes, httpHeaders)
            break
          } catch (error) {
            retryCountLocal++
          }
