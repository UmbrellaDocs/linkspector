  ...
  try {
    ...
    specifiedFiles.forEach((file) => {
      ...
      if (existsSync(filePath)) {
        if (!files.includes(filePath)) {
          ...
          console.warn(
            `Warning: The file "${file}" specified in the config is already included.`
          );
        }
      } else {
        console.warn(
          `Warning: The file "${file}" specified in the config does not exist.`
        );
      }
    });
    ...
    dirs.forEach((dir) => {
      ...
      if (existsSync(directory)) {
        ...
        console.error(
          `Warning: The directory "${directory}" specified in the config does not exist.`
        );
      }
    });
