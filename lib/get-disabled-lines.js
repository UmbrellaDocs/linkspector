...
function getNextNonEmptyLine(markdownText, commentLine) {
  const lines = markdownText.split('\n')
  for (let i = commentLine; i < lines.length; i++) {
    if (lines[i].trim() !== '') {
      return i + 1
    }
  }
...
function getDisabledLines(tree, markdownText) {
  ...
  for (const { directive, line } of comments) {
    switch (directive) {
      ...
      case 'linkspector-disable-next-line': {
        const nextLine = getNextNonEmptyLine(markdownText, line)
        if (nextLine !== null) {
          disabled.add(nextLine)
        }
        break
      }
