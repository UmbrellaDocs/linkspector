...
import { getDisabledLines } from './get-disabled-lines.js'
...
function extractMarkdownHyperlinks(markdownText, options) {
  ...
  const { isDisabled } = getDisabledLines(tree, markdownText)

  const links = []
  let anchorCount = 0
  visit(tree, ['link', 'definition', 'image', 'heading'], (node) => {
    if (node.type === 'heading') {
      anchorCount++
    } else if (!isDisabled(node.position.start.line)) {
      links.push(node)
    }
  })
  return { links: doReplacements(links, options), anchorCount }
}
